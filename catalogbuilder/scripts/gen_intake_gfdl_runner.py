...
def create_catalog_default(input_path=input_path,output_path=output_path):
    csv, json = gen_intake_gfdl.create_catalog(input_path=input_path,output_path=output_path, config=None, fill=True, filter_realm=None, filter_freq=None, filter_chunk=None, overwrite=False, append=False, slow=False, strict = False, verbose=False)
    return(csv,json)
