...
def compval(json_path,json_template_path, vocab, proper_generation, test_failure):
    ...
    ''' The Comprehensive Validator has two types of tests within it:
     ...
     This test validates catalogs against CMIP6 or GFDL controlled vocabulary (CV) as provided by particular JSON schemas per vocabulary type. CMIP6 CV's are found in the WCRP-CMIP/CMIP6_CVs github repository. GFDL CV's are found in the NOAA-GFDL/CMIP6_CVs github repository. Must use '--vocab' flag to enable this test.
